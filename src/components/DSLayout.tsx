...
export default function DSLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <header className="h-[80px] flex items-center px-4 gap-3 fixed top-0 left-0 right-0 z-50 border-b border-[#e0b86a]" style={{ backgroundColor: '#FDF1D0' }}>
        <button
      ...
      <div className="flex pt-[80px] min-h-screen">
        ...
        <aside id="mobile-sidebar" role="navigation" aria-label="Menu de navegação"
          className={`fixed top-[80px] bottom-0 left-0 w-64 bg-sidebar text-sidebar-foreground z-40 overflow-y-auto transition-transform duration-200 lg:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
          {renderNav(false)}
        </aside>

        {/* Desktop sidebar */}
        <aside id="desktop-sidebar" role="navigation" aria-label="Menu de navegação"
          className={`hidden lg:flex lg:flex-col sticky top-[80px] h-[calc(100vh-80px)] bg-sidebar text-sidebar-foreground overflow-y-auto transition-all duration-200 ${collapsed ? "w-16" : "w-64"}`}>
